...
from schemas.comment_schema import (CommentResponse, CommentUpdationForm, CommentCreationForm, CommentStatistic,
                                    CommentStatisticOneDay)
...
class CommentCrud:
    ...
    async def add_new_comment_to_post(self, post_id: int, background_tasks: BackgroundTasks,
                                      payload: CommentCreationForm, curr_user: User) -> CommentResponse:
        ...
        if post.is_blocked:
            ...
            is_blocked = post_crud.policy_check(title=payload.title, text=payload.text)

        obj_in = Comment(title=payload.title, text=payload.text, user_id=curr_user.id, post_id=post_id,
                         reply_to=payload.reply_to, is_blocked=is_blocked)
        ...
        if is_blocked:
            raise HTTPException(status_code=403,
                                detail="Your comment contains prohibited content and has been blocked.")
    ...
    async def get_all_comments_to_post(self, post_id: int) -> List[CommentResponse]:
        ...
        db_result = await self.db.execute(select(Comment).options(selectinload(Comment.post))
                                          .options(selectinload(Comment.user)).where(Comment.post_id == post_id))
    ...
    async def update_comment_to_post(self, post_id: int, comment_id: int,
                                     payload: CommentUpdationForm, curr_user: User) \
            -> CommentResponse:
    ...
    async def get_comments_daily_breakdown(self, date_from, date_to) -> CommentStatistic:
        ...
        statement = select(
            ...
            func.count().filter(Comment.is_blocked.is_(False)).label('created_comments'),
            func.count().filter(Comment.is_blocked.is_(True)).label('blocked_comments'),
        ...
        return CommentStatistic(comments_statistic_per_day=[CommentStatisticOneDay(
            date=el.get('date'),
            created_comments_amount=el.get('created_comments'),
            blocked_comments_amount=el.get('blocked_comments'),
            total_comments_amount=el.get('total'))
