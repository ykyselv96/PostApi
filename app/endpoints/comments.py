...
async def add_new_comment_to_post(payload: CommentCreationForm, post_id: int, background_tasks: BackgroundTasks, comment_crud: CommentCrud = Depends(get_comment_crud), curr_user: User = Depends(get_current_user)) -> CommentResponse:
    res = await comment_crud.add_new_comment_to_post(post_id=post_id, payload=payload, background_tasks=background_tasks, curr_user=curr_user)
    return res


@router.get("/{post_id}/comments", status_code=status.HTTP_200_OK, response_model = Page[CommentResponse])
async def get_all_comments_to_post(post_id: int, comment_crud: CommentCrud = Depends(get_comment_crud), params: Params = Depends(),
...
async def get_comment_to_post_by_comment_id(post_id: int, comment_id: int, comment_crud: CommentCrud = Depends(get_comment_crud),
                        curr_user=Depends(get_current_user)) -> Page[PostResponse]:
...
async def update_comment_to_post(payload: CommentUpdationForm, post_id: int, comment_id: int, comment_crud: CommentCrud = Depends(get_comment_crud),
                         curr_user=Depends(get_current_user)) -> CommentResponse:
    res = await comment_crud.update_comment_to_post(post_id=post_id, comment_id=comment_id, payload=payload, curr_user=curr_user)
    return res


@router.delete("{post_id}/comments/{comment_id}",  status_code=status.HTTP_200_OK, response_model=CommentResponse)
async def delete_comment(post_id: int, comment_id: int,  comment_crud: CommentCrud = Depends(get_comment_crud), curr_user=Depends(get_current_user)) -> PostResponse:
